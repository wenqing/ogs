...
static bool hasDensity(
    const std::unique_ptr<MaterialLib::Fluid::FluidProperty>& f_prop)
{
    if (f_prop == nullptr)
        return false;
    else
        return (f_prop->getName().find("density dependent") !=
                std::string::npos)
                   ? true
                   : false;
}
...
FluidPropertiesWithDensityDependentModels::
    ...
    : FluidProperties(std::move(density), std::move(viscosity),
      ...
      _is_density_depedent{{true,
                            hasDensity(_property_models[static_cast<unsigned>(
                                FluidPropertyType::Vicosity)]),
                            hasDensity(_property_models[static_cast<unsigned>(
                                FluidPropertyType::HeatCapacity)]),
                            hasDensity(_property_models[static_cast<unsigned>(
