...
#include "CreateHydroPhaseFieldProcess.h"
...
std::unique_ptr<Process> createHydroPhaseFieldProcess(
    MeshLib::Mesh& mesh,
    ...
    BaseLib::ConfigTree const& config)
{
    ...
    DBUG("Create HydroPhaseFieldProcess.");
    ...
    else  // staggered scheme.
    {
        ...
        for (auto const& variable_name :
             {"displacement"s, "pressure"s, "phasefield"s})
        {
            auto per_process_variables =
                findProcessVariables(variables, pv_config, {variable_name});
            process_variables.push_back(std::move(per_process_variables));
        }
        mechanics_related_process_id = 0;
        hydro_process_id = 1;
        phase_field_process_id = 2;

        variable_u = &process_variables[mechanics_related_process_id][0].get();
        variable_pres = &process_variables[hydro_process_id][0].get();
    ...
    DBUG("Associate pressure with process variable \'%s\'.",
         variable_pres->getName().c_str());

    if (variable_pres->getNumberOfComponents() != 1)
    {
        OGS_FATAL(
            "Pressure process variable '%s' is not a scalar variable but has "
            "%d components.",
            variable_pres->getName().c_str(),
            variable_pres->getNumberOfComponents());
    }
    ...
    auto const constitutive_relation_config =
        //! \ogs_file_param{prj__processes__process__HYDRO_PHASE_FIELD__constitutive_relation}
        config.getConfigSubtree("constitutive_relation");

    auto const phasefield_parameters_config =
        //! \ogs_file_param{prj__processes__process__HYDRO_PHASE_FIELD__phasefield_parameters}
        config.getConfigSubtree("phasefield_parameters");

    auto const type =
        //! \ogs_file_param{prj__processes__process__HYDRO_PHASE_FIELD__constitutive_relation__type}
    ...
    auto& residual_stiffness = findParameter<double>(
        ...
        //! \ogs_file_param_special{prj__processes__process__HYDRO_PHASE_FIELD__phasefield_parameters__residual_stiffness}
    ...
    auto& crack_resistance = findParameter<double>(
        phasefield_parameters_config,
        //! \ogs_file_param_special{prj__processes__process__HYDRO_PHASE_FIELD__phasefield_parameters__crack_resistance}
    ...
    auto& crack_length_scale = findParameter<double>(
        phasefield_parameters_config,
        //! \ogs_file_param_special{prj__processes__process__HYDRO_PHASE_FIELD__phasefield_parameters__crack_length_scale}
    ...
    auto& kinetic_coefficient = findParameter<double>(
        phasefield_parameters_config,
        //! \ogs_file_param_special{prj__processes__process__HYDRO_PHASE_FIELD__phasefield_parameters__kinetic_coefficient}
    ...
    auto& solid_density = findParameter<double>(
        config,
        //! \ogs_file_param_special{prj__processes__process__HYDRO_PHASE_FIELD__solid_density}
    ...
    Eigen::Matrix<double, DisplacementDim, 1> specific_body_force;
    {
        std::vector<double> const b =
            //! \ogs_file_param{prj__processes__process__HYDRO_PHASE_FIELD__specific_body_force}
    ...
    auto& residual_stiffness = findParameter<double>(
        phasefield_parameters_config,
        //! \ogs_file_param_special{prj__processes__process__HYDRO_PHASE_FIELD__phasefield_parameters__flow_rate}
        "flow_rate", parameters, 1);
    DBUG("Use \'%s\' as flow_rate.", flow_rate.name.c_str());
    ...
    HydroPhaseFieldProcessData<DisplacementDim> process_data{
        ...
        history_field,       specific_body_force, flow_rate};

    SecondaryVariableCollection secondary_variables;

    NumLib::NamedFunctionCaller named_function_caller(
        {"HydroPhaseField_displacement"});

    ProcessLib::createSecondaryVariables(config, secondary_variables,
                                         named_function_caller);

    return std::make_unique<HydroPhaseFieldProcess<DisplacementDim>>(
        mesh, std::move(jacobian_assembler), parameters, integration_order,
        std::move(process_variables), std::move(process_data),
        std::move(secondary_variables), std::move(named_function_caller),
        use_monolithic_scheme, mechanics_related_process_id, hydro_process_id,
        phase_field_process_id);
}

template std::unique_ptr<Process> createHydroPhaseFieldProcess<2>(
    MeshLib::Mesh& mesh,
    std::unique_ptr<ProcessLib::AbstractJacobianAssembler>&& jacobian_assembler,
    std::vector<ProcessVariable> const& variables,
    std::vector<std::unique_ptr<ParameterBase>> const& parameters,
...
template std::unique_ptr<Process> createHydroPhaseFieldProcess<3>(
    MeshLib::Mesh& mesh,
    std::unique_ptr<ProcessLib::AbstractJacobianAssembler>&& jacobian_assembler,
    std::vector<ProcessVariable> const& variables,
    std::vector<std::unique_ptr<ParameterBase>> const& parameters,
