...
void StaggeredHTFEM<ShapeFunction, IntegrationMethod, GlobalDim>::
    assembleHydraulicEquation(double const t, std::vector<double>& local_M_data,
                              ...
                              LocalCoupledSolutions const& coupled_term)
{
    auto const& local_p = coupled_term.local_coupled_xs[1];
    auto const local_matrix_size = local_p.size();
    // This assertion is valid only if all nodal d.o.f. use the same shape
    // matrices.
    assert(local_matrix_size == ShapeFunction::NPOINTS);

    auto const& local_T1 = coupled_term.local_coupled_xs[0];
    auto const& local_T0 = coupled_term.local_coupled_xs0[0];
                          ...
                          std::vector<double>& cache) const
{
    ...
    auto const local_xs =
        getCurrentLocalSolutions(*(this->_coupling_term), indices);
