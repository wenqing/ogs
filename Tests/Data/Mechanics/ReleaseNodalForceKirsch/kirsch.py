...
#
# With the standard approach, the problem is steady-state, requiring only one time step. With the release nodal force approach, the use of the non-initial equilibrium residual is required, which means the constant traction boundary condition can be ignored. In addition, we assume that the nodal forces at the arc-type boundary are scaled by a linear time-decay factor that varies from 1 to 0 over a period of 2 days. After 2 days, the stress solutions obtained by this approach should be identical to those from the standard approach, and should closely match the analytical solution.
#
...
#
# Additionally, the benchmark is evaluated using linear elements, which result in significant stress errors. Therefore, linear elements should be avoided in stress analyses to ensure accurate results.
